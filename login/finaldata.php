...
$csvnames = implode(", ", $filterdistricts) . ", " . implode(", ", $byStates);
...
if (isset($_POST['get-data'])) {
    ...
    $byStates = isset($_POST['filterState']) ? $_POST['filterState'] : [];
    $filterdistricts = isset($_POST['filterdistrict']) ? $_POST['filterdistrict'] : [];

    if (!empty($byStates)) {
        $stateFilter = implode("', '", $byStates);
        $filters[] = " state IN ('$stateFilter')";
    }
    if (!empty($filterdistricts)) {
        $districtFilter = implode("', '", $filterdistricts);
        $filters[] = " district IN ('$districtFilter')";
    }
    ...
    <div class="container">
        <form action="" method="POST" class="filterform d-flex flex-wrap gap-1">
            ...
            <select name="filterdistrict" name="filterCity" class="form-select form-select-sm"
                aria-label="Small select example" id="districtSelect" onchange="selectingtehsil(this)" multiple>
...
" class="text-center position-absolute  start-50 translate-middle">
                        ...
                        <?php
                        if (isset($_POST['filterState'])) echo $_POST['filterState'];
                        ?>
                    </th>
                </tr>
            </thead>
                ...
                if (isset($_POST['get-data'])) {
                    ...
                    if ($totalresult > 0) {
                        while ($row = mysqli_fetch_array($result)) {
                            ...
                            echo '</tr>';
                            $dataheading = ucwords(strtolower($row['state']));
                        }
                    } else {
