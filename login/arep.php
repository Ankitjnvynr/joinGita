...
<body>
  ...
  // Add Header (replace with desired position)
  doc.text("GIEO Gita Report", 20, 10);

  // Define background color for header row (optional)
  const headerBackgroundColor = '#f7f7f7'; // Example light gray

  doc.autoTable({
    html: `#${tableId}`,
    theme: 'plain',
    // Add custom styling for header row if needed (refer to jspdf-autotable documentation)
    didDrawPage: function (data) {
      // Potential logic to set background color for header row here
      if (data.pageNumber === 1) {
        const headerHeight = // Get header row height
  ...
  doc.save(`GIEO_Gita_Report_${formattedDate}.pdf`);
}
    </script>
